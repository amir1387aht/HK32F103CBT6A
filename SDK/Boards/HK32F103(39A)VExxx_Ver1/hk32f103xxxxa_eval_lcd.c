...
* @filename:  hk32f103xxxxa_eval_lcd.c
* @brief:     This file provides all the LCD(ili9341) firmware functions.
* @author:    AE Team
* @version:   V1.0.0/2023-11-01
*             1.Initial version
* @log:
*******************************************************************************/
...
#define DEBUG_DELAY()       ILI9341_Delay(10)
...
static void ILI9341_GPIO_Config ( void )
{
    ...
    RCC_APB2PeriphClockCmd (
        /*控制信号*/
        ILI9341_CS_CLK | ILI9341_DC_CLK | ILI9341_CS1_CLK | ILI9341_CS2_CLK | ILI9341_WR_CLK | ILI9341_RD_CLK |
    ...
    GPIO_InitStructure.GPIO_Pin = ILI9341_CS1_PIN;
    GPIO_Init ( ILI9341_CS1_PORT, & GPIO_InitStructure );
    GPIO_InitStructure.GPIO_Pin = ILI9341_CS2_PIN;
    GPIO_Init ( ILI9341_CS2_PORT, & GPIO_InitStructure );
    /* 配置LCD复位RST控制管脚*/
...
static void ILI9341_FSMC_Config ( void )
{
    ...
    FSMC_ReadWriteTimingStruct.FSMC_DataLatency = 2;
...
void ILI9341_DrawPicture( uint16_t usX1, uint16_t usY1, uint16_t usX2, uint16_t usY2, uint16_t *pic )
{
    int32_t lDelta_X = 0, lDelta_Y = 0;
    int32_t pic_cnt, i;
    ...
    ILI9341_OpenWindow ( usX1, usY1, lDelta_X, lDelta_Y );
    ILI9341_SetCursor ( usX1, usY1 );
    ILI9341_Write_Cmd ( CMD_SetPixel );
    pic_cnt = lDelta_X * lDelta_Y;

    for (i = 0; i < pic_cnt; i++)
    {
        ILI9341_Write_Data ( *pic++ );
    }
}
/**
...
void Show_pic(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t *data)
{
    /* 开一个图片大小的窗口*/
	...
	for (int i = 0; i < height; i ++ )
    {
        ...
        {
            //一行中第K个像素的起点
            DataPic = (data[i * 640 + 2 * j + 1] << 8 ) | data[i * 640 + 2 * j];
            ILI9341_Write_Data ( DataPic); //写入LCD-GRAM
        }
    }
}

void Show_pic16(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t *data)
{
    /* 开一个图片大小的窗口*/
    ILI9341_OpenWindow(x, y, width, height);
    ILI9341_Write_Cmd (CMD_SetPixel );
