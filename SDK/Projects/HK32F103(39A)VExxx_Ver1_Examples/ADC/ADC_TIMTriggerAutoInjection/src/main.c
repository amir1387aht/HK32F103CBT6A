...
/** @addtogroup ADC_TIMTriggerAutoInjection
  * @{
  */
...
static __IO uint16_t ADC_RegularConvertedValueTab[32];
...
int main(void)
{
    ...
    printf("\r\nRegular channel10 data:\r\n");
    for(int i = 0 ; i<32 ;i++)
    {
        printf("%03X\t",ADC_RegularConvertedValueTab[i]);
    }
    printf("\r\nInjecte channel11 data:\r\n");
    for(int i = 0 ; i<32 ;i++)
    {
        printf("%03X\t",ADC_InjectedConvertedValueTab[i]);
    }
    while (1)
    {
...
static void GPIO_Configuration(void)
{
    ...
    /* Configure PC.01 and PC.00 (ADC Channel11 and Channel10) as analog input */
