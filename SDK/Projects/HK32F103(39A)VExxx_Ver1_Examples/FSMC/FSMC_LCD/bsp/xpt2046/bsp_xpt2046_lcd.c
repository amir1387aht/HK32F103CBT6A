...
* @filename:  main.c
* @brief:     This file provides the xpt2046 driver.
* @author:    AE Team
...
uint8_t XPT2046_Touch_Calibrate ( uint8_t LCD_Mode )
{
    ...
    usTest_x = (uint16_t)(( ( CalibrationFactor.An * strScreenSample[3].x ) + ( CalibrationFactor.Bn * strScreenSample[3].y ) + CalibrationFactor.Cn ) / CalibrationFactor.Divider);        //取一个点计算X值
    usTest_y = (uint16_t)(( ( CalibrationFactor.Dn * strScreenSample[3].x ) + ( CalibrationFactor.En * strScreenSample[3].y ) + CalibrationFactor.Fn ) / CalibrationFactor.Divider);    //取一个点计算Y值
    usGap_x = (uint16_t)(( usTest_x > strCrossCoordinate[3].x ) ? ( usTest_x - strCrossCoordinate[3].x ) : ( strCrossCoordinate[3].x - usTest_x ));   //实际X坐标与计算坐标的绝对差
    usGap_y = (uint16_t)(( usTest_y > strCrossCoordinate[3].y ) ? ( usTest_y - strCrossCoordinate[3].y ) : ( strCrossCoordinate[3].y - usTest_y ));   //实际Y坐标与计算坐标的绝对差
...
//touch_para addr：0x807F800
#ifdef __IAR_Embedd_
  #define TOUCH_PARA_ADDR                 0x807F800
  #pragma location = TOUCH_PARA_ADDR
  __root strType_XPT2046_TouchPara strXPT2046_TouchPara_flash[8] ;
  #define TOUCH_PARA_FLAG_ADDR                0x807F900
  #pragma location = TOUCH_PARA_FLAG_ADDR
  __root uint32_t touch_para_flag_value ;
#else
  #define TOUCH_PARA_ADDR                 0x807F800
  strType_XPT2046_TouchPara strXPT2046_TouchPara_flash[8] __attribute__((at(TOUCH_PARA_ADDR)));
  #define TOUCH_PARA_FLAG_ADDR                0x807F900
  uint32_t touch_para_flag_value __attribute__((at(TOUCH_PARA_FLAG_ADDR)));
#endif
uint8_t get_touch_para(void)
{
    if (touch_para_flag_value == FLASH_TOUCH_PARA_FLAG_VALUE)
    {
        memcpy(strXPT2046_TouchPara, strXPT2046_TouchPara_flash, sizeof(strXPT2046_TouchPara_flash));
        return FLASH_TOUCH_PARA_FLAG_VALUE;
    }

    return 0;
}

void save_touch_para(void)
{
    uint32_t addr = 0x807F800;
    uint32_t *pdata = (uint32_t *)strXPT2046_TouchPara;
    uint16_t i, data_lens = sizeof(strXPT2046_TouchPara_flash) / 4;
    FLASH_Unlock();
    FLASH_ErasePage(TOUCH_PARA_ADDR);

    for (i = 0; i < data_lens; i++)
    {
        uint32_t data = *pdata;
    ...
    FLASH_ProgramWord(TOUCH_PARA_FLAG_ADDR, FLASH_TOUCH_PARA_FLAG_VALUE);
    FLASH_Lock();
}

/**
  * @brief  从FLASH中获取 或 重新校正触摸参数（校正后会写入到FLASH中）
...
uint8_t XPT2046_Get_TouchedPoint ( strType_XPT2046_Coordinate * pDisplayCoordinate, strType_XPT2046_TouchPara * pTouchPara )
{
    ...
    {
        pDisplayCoordinate ->x = (uint16_t)(( ( pTouchPara[LCD_SCAN_MODE].dX_X * strScreenCoordinate.x ) + ( pTouchPara[LCD_SCAN_MODE].dX_Y * strScreenCoordinate.y ) + pTouchPara[LCD_SCAN_MODE].dX ));
        pDisplayCoordinate ->y = (uint16_t)(( ( pTouchPara[LCD_SCAN_MODE].dY_X * strScreenCoordinate.x ) + ( pTouchPara[LCD_SCAN_MODE].dY_Y * strScreenCoordinate.y ) + pTouchPara[LCD_SCAN_MODE].dY ));
    }
