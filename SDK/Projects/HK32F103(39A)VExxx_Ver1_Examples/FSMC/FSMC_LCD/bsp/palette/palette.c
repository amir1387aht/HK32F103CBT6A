...
* @filename:  main.c
* @brief:     This file provides the paletle function.
* @author:    AE Team
...
void Draw_Trail(int16_t pre_x, int16_t pre_y, int16_t x, int16_t y, Brush_Style* brush)
{
    ...
    if (x > PALETTE_START_X && pre_x > PALETTE_START_X )
    {
        switch (brush->shape) /*根据画刷参数描绘不同的轨迹*/
        {
            ...
            case LINE_2_PIXCEL:
				//画板左边界
                if (x - 1 < PALETTE_START_X || pre_x - 1 < PALETTE_START_X) 
				{
                    break;
				}
            ...
            case LINE_4_PIXCEL:
				//画板左边界
                if (x - 2 < PALETTE_START_X || pre_x - 2 < PALETTE_START_X) 
				{
                    break;
				}
            ...
            case LINE_8_PIXCEL:
				//画板左边界
                if (x - 4 < PALETTE_START_X || pre_x - 4 < PALETTE_START_X) 
                {    
					break;
				}
            ...
            case LINE_16_PIXCEL:
				//画板左边界
                if (x - 8 < PALETTE_START_X || pre_x - 8 < PALETTE_START_X) 
                {    
					break;
				}
            ...
            case LINE_20_PIXCEL:
				//画板左边界
                if (x - 10 < PALETTE_START_X || pre_x - 10 < PALETTE_START_X) 
                {    
					break;
				}
            ...
            /*橡皮功能*/
            case RUBBER:
				// PALETTE_START_X:画板左边界 LCD_X_LENGTH: 液晶左右边界 LCD_Y_LENGTH: 液晶上下边界
                if (x - 20 < PALETTE_START_X || x + 20 > LCD_X_LENGTH || x - 20 < 0 || y + 20 > LCD_Y_LENGTH || y - 20 < 0) 
                {    
					break;
				}
    ...
    switch (ptr->para)
    {
        ...
        case RUBBER:
            ...
            *中文字体大小是24*24的，需要其它字体请自行制作字模*/
            ...
            LCD_SetFont(&Font8x16);
            ILI9341_DispString_EN_CH( ptr->start_x + (ptr->end_x - ptr->start_x - 16 * 2) / 2,
                                      ptr->start_y + ((ptr->end_y - ptr->start_y - 24 - 24 - 16) / 2) + 24,
                                      "eraser");
            break;
    }
}
...
static void LCD_DrawUniLineCircle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint8_t thick )
{
    ...
    for (curpixel = 0; curpixel <= numpixels; curpixel++)
    {
        /*判断边界
		  液晶左右边界 || 液晶上下边界 */
        if (x + thick > LCD_X_LENGTH || x - thick < 0 || y + thick > LCD_Y_LENGTH || y - thick < 0  ) 
		{
            continue;
		}
